...
export default class CabinetNode extends WamNode {
  static async addModules(context,moduleId,wasmUrl){await super.addModules(context,moduleId);await addFunctionModule(context.audioWorklet,getCabinetProcessor,moduleId);const r=await fetch(wasmUrl);if(!r.ok)throw Error(`Cannot load Cabinet WASM: ${r.status}`);this.wasmModule=await WebAssembly.compile(await r.arrayBuffer());}
  constructor(module){super(module,{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[1],channelCount:1,channelCountMode:'explicit',channelInterpretation:'discrete',processorOptions:{wasmModule:CabinetNode.wasmModule,useSab:false}});this._id=0;this._pending=new Map();this._ir=null;this._trimByIr={};this._gui=null;}
  ...
  async loadImpulseResponse(samples,name='cabinet.wav',assetId=name,metadata={}){const analysis=analyzeImpulseResponse(samples);const copy=new Float32Array(samples);const trimDb=this._trimByIr[assetId]??0;const result=await this._request('load',{samples:copy.buffer,name,compensation:analysis.compensation,analysis},[copy.buffer]);this._ir={id:assetId,name,samples:Array.from(samples),analysis,metadata};await this.setParameterValues({irTrim:{id:'irTrim',value:trimDb,normalized:false}});this._gui?.setIrStatus(name,result,analysis,trimDb);return {...result,analysis,trimDb};}
  ...
  async getState(){const s=await super.getState();return {parameterValues:s.parameterValues,ir:this._ir,trimByIr:this._trimByIr,stateVersion:1};}
  async setState(s){this._trimByIr=s.trimByIr||{};if(s.ir?.samples)await this.loadImpulseResponse(Float32Array.from(s.ir.samples),s.ir.name,s.ir.id||s.ir.name,s.ir.metadata||{});await super.setState({parameterValues:s.parameterValues||{}});this._gui?.syncParameters(await super.getParameterValues(false));}
