...
export default class CabinetNode extends WamNode {
  ...
  async loadImpulseResponse(samples,name='cabinet.wav',assetId=name){const analysis=analyzeImpulseResponse(samples);const copy=new Float32Array(samples);const trimDb=this._trimByIr[assetId]??0;const result=await this._request('load',{samples:copy.buffer,name,compensation:analysis.compensation,analysis},[copy.buffer]);this._ir={id:assetId,name,samples:Array.from(samples),analysis};await this.setParameterValues({irTrim:{id:'irTrim',value:trimDb,normalized:false}});this._gui?.setIrStatus(name,result,analysis,trimDb);return {...result,analysis,trimDb};}
  ...
  async setState(s){this._trimByIr=s.trimByIr||{};if(s.ir?.samples)await this.loadImpulseResponse(Float32Array.from(s.ir.samples),s.ir.name,s.ir.id||s.ir.name);await super.setState({parameterValues:s.parameterValues||{}});this._gui?.syncParameters(await super.getParameterValues(false));}
