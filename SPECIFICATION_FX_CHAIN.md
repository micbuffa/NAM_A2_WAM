...
- Input and output endpoint panels remain outside the scrolling strip and sticky in the rack. Both are 193 px tall and 161 px wide, matching the cards including their toolbars. Inputs retain their small analogue-style needle meters and gain controls. Outputs use a compact mixer strip: a vertical volume fader with a rectangular mixer-style handle, an adjacent narrow segmented level meter, Reset and Mute buttons, and a horizontal L/R pan slider. Device selectors remain shared in the rack header. Both meter styles measure RMS dBFS from −60 to 0 and retain a one-second clipping indication. Output panels keep their existing dimensions.
...
- Visible input meters remain post-input-gain in independent mode; B has no input meter panel in split mode. Output meters measure after lane output gain, mute and pan, before shared mix compensation. The final clip indicator observes the compensated sum. Retain green → orange halos and the one-second red peak indication.
